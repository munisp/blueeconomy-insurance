...
from insurance.crypto.eddsa import (
    JwsError,
    KeyDirectory,
    b64u_decode,
    b64u_encode,
...
from insurance.events.envelope import (
    EnvelopeError,
    build_envelope,
    sign_envelope,
    verify_envelope,
...
def _envelope():
    return build_envelope(
        event_type="insurance.policy.v1",
...
def test_envelope_sign_verify(signing_key, directory):
    ...
    resource = verify_envelope(signed, directory, expected_event_type="insurance.policy.v1")
...
def test_envelope_structural_fail_closed(signing_key, directory):
    ...
    with pytest.raises(EnvelopeError) as exc:
        verify_envelope(bad_type, directory, expected_event_type="insurance.claim.v1")
...
def test_placeholder_key_refused(tmp_path, monkeypatch):
    from insurance.crypto.eddsa import generate_pkcs8_pem, load_signing_key

    monkeypatch.setenv("INSURANCE_ALLOW_PERMISSIVE_KEY_FILE", "1")
    p = tmp_path / "key.pem"
...
def test_key_permissions_enforced(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("INSURANCE_ALLOW_PERMISSIVE_KEY_FILE", raising=False)
    p = tmp_path / "key.pem"
