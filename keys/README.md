...
```sh
python -c "from insurance.crypto.eddsa import generate_pkcs8_pem; \
  open('keys/ed25519_pkcs8.pem','wb').write(generate_pkcs8_pem())"
chmod 600 keys/ed25519_pkcs8.pem
```

Point `INSURANCE_SIGNING_KEY_PATH` at it. The service refuses to boot when
...
```sh
python - <<'EOF'
from insurance.crypto.eddsa import load_signing_key
import os
os.environ["INSURANCE_ALLOW_PERMISSIVE_KEY_FILE"] = "1"
