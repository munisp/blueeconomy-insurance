...
from insurance.db import dispose_engine, init_engine, session
from insurance.models import OutboxMessage, utcnow
...
async def publish_forever() -> None:
    ...
    if not settings.kafka_configured:
        raise RuntimeError("INSURANCE_KAFKA_BOOTSTRAP_SERVERS is required for the outbox publisher")
    if not settings.database_url:
        raise RuntimeError("INSURANCE_DATABASE_URL is required for the outbox publisher")
