...
def load_signing_key(path: str, kid: str) -> SigningKey:
    ...
    if _is_production() and os.environ.get("INSURANCE_ALLOW_PERMISSIVE_KEY_FILE"):
        ...
        raise JwsError(
            ...
            "INSURANCE_ALLOW_PERMISSIVE_KEY_FILE is forbidden when ENV=production",
        )
    p = Path(path)
    if not p.is_file() or p.is_symlink():
        raise JwsError("key-unavailable", f"signing key {path} is not a regular non-symlink file")
    if os.stat(p).st_mode & 0o077 and os.environ.get("INSURANCE_ALLOW_PERMISSIVE_KEY_FILE") != "1":
