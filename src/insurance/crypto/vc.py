...
def build_policy_credential(
    *,
    ...
    """The marine insurance policy credential (unsigned; proof attached by
    issue_proof).

    MINIMAL DISCLOSURE: a policy credential is presented to counterparties
    (customs, port authorities, shippers) and must not disclose commercial
    terms. credentialSubject therefore carries exactly the verifiable-cover
    fields — the Luhn-gated policy number, product identity, corridor,
    validity window and a ``declarationRef`` through which cover details
    remain resolvable by AUTHORIZED parties only (policy-gated read path).
    Premium, insured value and assured identifiers are deliberately NOT
    embedded in the public credential.
    """
    return {
        "@context": list(VC_CONTEXT),
        "id": credential_id,
        "type": ["VerifiableCredential", "MarineInsurancePolicy"],
        "issuer": issuer_did,
        "validFrom": valid_from,
        "validUntil": valid_until,
        "credentialSubject": {
            "id": f"{credential_id}#policy",
