"""W3C Bitstring Status List v1.0 for policy revocation/suspension state.

Each policy's credential carries two ``BitstringStatusListEntry`` statuses —
purposes ``revocation`` and ``suspension`` — each pointing into its own
status-list credential at the policy's assigned index. Policy state is thus
expressed in the published status lists, NOT only as a database-row status
(the database row is the service's working state; the signed status lists
are the verifier-facing truth).
...
from insurance.crypto.vc import VC_CONTEXT, issue_proof, utc_now_iso
...
def status_entry(list_credential_id: str, index: int, purpose: str) -> dict[str, Any]:
    """A BitstringStatusListEntry for embedding in a policy credential."""
    if purpose not in PURPOSES:
